...
bool scene_traverser::for_each(pugi::xml_node& node)
{
    if(std::strncmp(node.name(),"material",8) == 0)
    {
        ...
        scene->material_list.insert(std::pair<std::string, Material*>(id, m));
    }
    else if(std::strncmp(node.name(),"object3D",8) == 0)
    {
        ...
        if (type == "plane")
        {
            ...
            object = new Plane(P0, P1, P2, scene->material_list[material_id]);
        }
        if (type == "sphere")
        {
            glm::vec3 position(
            ...
            object = new Sphere(position, radius, scene->material_list[material_id]);
        }
        if (type == "mesh")
        {
            ...
            object = new Mesh(mesh_transform, file_path.c_str(), scene->material_list[material_id]);
        }
        scene->object_list.push_back(object);
    }
    else if(std::strncmp(node.name(),"light_source",12) == 0)
    {
        ...
        scene->light_list.push_back(lamp);
    }
    return true; // Continue traversal
}